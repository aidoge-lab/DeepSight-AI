            ...
            if statement and not statement.startswith("--"):
                try:
                    ...
                    if failed_statements <= 10:  # Only log first 10 errors
                        logger.warning(
                            f"Warning executing statement in {file_path}: {e}"
                        )
                        logger.warning(f"Statement preview: {statement[:100]}...")
                    elif failed_statements == 11:
                        logger.warning(
                            f"Suppressing further error messages for {file_path}..."
                        )
...
def setup_database(db_path: str, data_dir: str) -> None:
    ...
    logger.info(
        f"Found {len(schema_files)} schema files and {len(insert_files)} insert files"
    )
