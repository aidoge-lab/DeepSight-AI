...
def test_data_integrity(conn: sqlite3.Connection) -> None:
    ...
    if cursor.fetchone():
        ...
        cursor.execute("SELECT COUNT(*) as count FROM ai_models WHERE model IS NULL OR model = ''")
        ...
        cursor.execute("SELECT COUNT(*) as count FROM ai_models WHERE publication_date IS NOT NULL")
        valid_dates = cursor.fetchone()['count']
        logger.info(f"✓ {valid_dates} rows have valid publication dates")
        
        # Test for parameter ranges
        cursor.execute("SELECT MIN(parameters) as min_params, MAX(parameters) as max_params FROM ai_models WHERE parameters IS NOT NULL")
        param_range = cursor.fetchone()
        if param_range['min_params'] and param_range['max_params']:
            logger.info(f"✓ Parameter range: {param_range['min_params']:,} to {param_range['max_params']:,}")
        
        # Test for organization data
        cursor.execute("SELECT COUNT(DISTINCT organization) as org_count FROM ai_models WHERE organization IS NOT NULL")
        org_count = cursor.fetchone()['org_count']
        logger.info(f"✓ {org_count} unique organizations")
        
        # Test for frontier models
        cursor.execute("SELECT COUNT(*) as count FROM ai_models WHERE frontier_model = 1")
...
def run_sample_queries(conn: sqlite3.Connection) -> None:
    ...
    for row in cursor.fetchall():
        logger.info(f"  - {row['model']} ({row['organization']}): {row['parameters']:,} parameters")
