...
def generate_insert_statement(row):
    ...
    values.append(
        convert_to_sql_number(row.get("Training compute (FLOP)"), is_decimal=True)
    )
    ...
    values.append(
        convert_to_sql_number(row.get("Training time (hours)"), is_decimal=True)
    )
    ...
    values.append(
        convert_to_sql_number(row.get("Hardware utilization"), is_decimal=True)
    )

    # Cost information
    values.append(
        convert_to_sql_number(
            row.get("Training compute cost (2023 USD)"), is_decimal=True
        )
    )
    ...
    values.append(
        convert_to_sql_number(row.get("Finetune compute (FLOP)"), is_decimal=True)
    )
